...
export class RowMenu extends BaseMenu {
  ...
  async showFilterRows(sheet, schema, returnCallback) {
    ...
    const jmesQuery = await this.showQuerySelection(sheet, schema)
  ...
  async showQuerySelection(sheet, schema) {
    try {
      ...
      if (queryChoice.startsWith('saved-')) {
        ...
        if (selectedQuery) {
          this.sheetOps.setLastJmesQuery(selectedQuery.JMESPathQuery)
      ...
      if (queryChoice === 'custom') {
        ...
        const customQuery = await input({
          ...
          default: this.sheetOps.getLastJmesQuery(),
          validate: (input) => {
            // Allow empty input
        ...
        this.sheetOps.setLastJmesQuery(queryText)

        // Ask if user wants to save this query
        if (queryText) {
          await this.sheetOps.saveQuery(sheet, schema, queryText)
        }
